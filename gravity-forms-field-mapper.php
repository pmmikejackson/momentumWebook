...
if ( $mwm_enable_mapper_direct_send ) {
	...
	add_filter(
		...
		function ( $request_data, $feed, $entry, $form ) {
	...
	add_action(
		'gform_after_submission_10',
		function ( $entry, $form ) use ( $mwm_mapper_direct_send ) {
			$mwm_mapper_direct_send( $entry, 10 );
		},
		10,
		2
	);
	add_action(
		...
		function ( $entry, $form ) use ( $mwm_mapper_direct_send ) {
			$mwm_mapper_direct_send( $entry, 11 );
		},
		10,
		2
	);
	add_action(
		'gform_after_submission_12',
		function ( $entry, $form ) use ( $mwm_mapper_direct_send ) {
			$mwm_mapper_direct_send( $entry, 12 );
		},
		10,
		2
	);
	add_action(
		...
		function ( $entry, $form ) use ( $mwm_mapper_direct_send ) {
			$mwm_mapper_direct_send( $entry, 1 );
		},
		10,
		2
	);
	add_action(
		'gform_after_submission_2',
		function ( $entry, $form ) use ( $mwm_mapper_direct_send ) {
			$mwm_mapper_direct_send( $entry, 2 );
		},
		10,
		2
	);
	add_action(
		'gform_after_submission_3',
		function ( $entry, $form ) use ( $mwm_mapper_direct_send ) {
			$mwm_mapper_direct_send( $entry, 3 );
		},
		10,
		2
	);
