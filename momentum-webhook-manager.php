		...
		<form method="post" action="<?php echo admin_url( 'admin.php?page=mwm-webhook-manager' ); ?>">
			...
			<div class="tablenav top">
				...
				<div class="tablenav-pages">
					<span class="displaying-num"><?php printf( '%d items', $total_count ); ?></span>
					<?php
...
function mwm_settings_page() {
	...
	$default_type       = get_option( 'mwm_default_type', 'prospect' );
	?>
				...
				<tr>
					...
					<td>
						<select name="default_type">
							<option value="" <?php selected( $default_type, '' ); ?>>(Blank)</option>
							<option value="prospect" <?php selected( $default_type, 'prospect' ); ?>>Prospect</option>
							<option value="insured" <?php selected( $default_type, 'insured' ); ?>>Insured</option>
						</select>
						<p class="description">Default value for the 'type' parameter sent to NowCerts API. Most applications are prospects.</p>
					</td>
				</tr>
			</table>
...
function mwm_send_entry_to_webhook( $entry_id, $form_id, $manual = false ) {
	...
	// Add type parameter for NowCerts API
	$default_type = get_option( 'mwm_default_type', 'prospect' );
	if ( ! empty( $default_type ) ) {
		$payload['type'] = $default_type;
	}

	// TODO: Option 5 - Smart Detection for type parameter
	// Future enhancement: Analyze form data to auto-detect type:
	// - Check for existing policy numbers (insured)
	// - Look for renewal-specific fields (insured)
	// - Detect new application patterns (prospect)
	// - Form-specific logic (e.g., Security Guard forms = prospect)
	// Implementation would replace the static default above with contextual logic
 ...
 */
function mwm_get_webhook_statistics( $use_cache = true ) {
	if ( ! class_exists( 'GFAPI' ) ) {
		return array();
	}

	// Check for cached stats if caching is enabled
	if ( $use_cache ) {
		$cached_stats = get_transient( 'mwm_webhook_statistics' );
	...
	foreach ( $form_ids as $form_id ) {
		...
		$total = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*) FROM $lead_table WHERE form_id = %d AND status = 'active'",
				$form_id
			)
		);
		...
		$sent = $wpdb->get_var(
			$wpdb->prepare(
             ...
             FROM $lead_table e
             INNER JOIN $meta_table m ON e.id = m.entry_id
             WHERE e.form_id = %d 
             AND e.status = 'active'
             AND m.meta_key = 'mwm_webhook_status'
             AND m.meta_value = 'sent'",
				$form_id
			)
		);
	...
	set_transient( 'mwm_webhook_statistics', $stats, 60 );
...
function mwm_dashboard_widget_content() {
	...
	<div class="mwm-dashboard-widget">
		...
		<div style="text-align: right; margin-bottom: 10px;">
			<a href="<?php echo admin_url( 'index.php' ); ?>" class="button-link" style="font-size: 12px;">↻ Refresh</a>
		</div>
